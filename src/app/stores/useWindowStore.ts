...
export const useWindowStore = create<WindowStore>((set, get) => ({
  ...
  openWindow: (app) => {
    ...
    // 새 창 열기 (가장 높은 z-index + 1, WINDOW_BASE부터 시작)
    const maxZ: number =
      windows.length > 0
  ...
  closeWindow: (id) => {
    set((state) => {
      const remainingWindows: WindowState[] = state.windows.filter(
        (w) => w.id !== id,
      );
      ...
      if (state.activeWindowId === id) {
        if (remainingWindows.length > 0) {
          const topWindow: WindowState = [...remainingWindows].sort(
  ...
  focusWindow: (id) => {
    ...
    // Window z-index는 WINDOW_MAX로 제한 (위젯보다 항상 높게 유지)
    const nextZ = Math.min(maxZ + 1, Z_INDEX.WINDOW_MAX);
